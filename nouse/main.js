...
const { RestClientV5, WebsocketClient, WS_KEY_MAP} = require('bybit-api');
const { v4: uuidv4 } = require('uuid');
const status_1 = require('./status1');
...
async function setWebsocket(){
  ...
  ws.subscribeV5('order', 'option');
  ws.subscribeV5('tickers.ETH-19SEP25-4400-C-USDT', 'option');
...
async function orderEventHandle(res){
  ...
  switch (productStatus) {
    case 1:
      ...
      // Object.keys(productStatus.orderListObj_open).forEach(key => {

      //   const tempObj = productStatus.orderListObj_open[key];

      //   const numberOfKeys = Object.keys(productStatus.orderListObj_open).length;
        
      // });

      //이거는 그냥 위에서 재주문하는게낳을듯
      const orderList = Object.values(productStatus.orderListObj_open) 

      let isTotalOrderfilled = checkTotalOrderfilled(orderList)

      if(isTotalOrderfilled){
        productStatus.position_status = 2
      }{//주문이 채워지지않은경우 productStatus.orderListObj_open_orderId를 통해 재주문 order qty leavesQty
...
function checkTotalOrderfilled(orderList) {

  const orders = orderList;

  const isTotalOrderfilled = orders.length >= 2 && orders.every(order => order.orderStatus === 'Filled');

  return isTotalOrderfilled;
}
